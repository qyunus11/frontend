...
public class DoktorEgzersizEkleController {
    ...
    private void handleKaydet() {
        ...
        new Thread(() -> {
            try {
                ApiClient.doktorEgzersizEkle(patientId, doctorId, date, type, applied);
                Platform.runLater(() -> {
                    AlertUtils.showInfo("Başarılı", "Egzersiz başarıyla kaydedildi!");
